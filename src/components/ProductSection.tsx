...
const ProductSection = ({ title, subtitle, description, imageSrc, imageAlt, videoSrc, backgroundVideo, reverse = false, imageClassName, imageContainerClassName, backgroundVideoScale, backgroundVideoPosition, backgroundVideoOffset, textCenter = false, textClassName, imageRightAbsolute = false }: ProductSectionProps) => {
  return (
    <section className={`py-8 sm:py-10 md:py-14 px-4 sm:px-6 relative ${imageRightAbsolute ? 'overflow-visible md:overflow-hidden' : 'overflow-hidden'}`}>
      {backgroundVideo && (
        <div className="absolute inset-0 z-0 overflow-hidden">
          {backgroundVideoPosition === 'center-top' ? (
            ...
            <video 
              ...
              style={backgroundVideoOffset ? { objectPosition: `left ${backgroundVideoOffset}` } : { objectPosition: 'left center' }}
            />
          )}
      ...
      {/* Imagem posicionada absolutamente à direita */}
      {imageRightAbsolute && imageSrc && (
        <div className="absolute right-4 sm:right-6 md:right-8 lg:right-10 xl:right-12 top-1/2 -translate-y-1/2 z-20 hidden md:flex items-center justify-center">
          <img 
            src={imageSrc} 
            alt={imageAlt}
      ...
      <div className={`container mx-auto ${imageRightAbsolute ? 'max-w-4xl md:max-w-5xl pr-0 md:pr-[200px] lg:pr-[220px]' : 'max-w-5xl'} relative z-10`}>
        {imageRightAbsolute ? (
          // Layout sem grid quando imagem está absoluta
          <div className={`${textCenter ? 'text-center' : ''} ${textClassName || ''}`}>
